...
public class CameraKeyframe extends Keyframe<Minecraft> {
    ...
    public CameraKeyframe(Entity entity) {
        this(entity.position().toVector3f(), new Quaternionf().rotationYXZ((float) Math.toRadians(entity.getYRot()), (float) Math.toRadians(entity.getXRot()), 0.0f));
    }

    public CameraKeyframe(Vector3f position, Quaternionf quaternion) {
    ...
    public static class TypeAdapter implements JsonSerializer<CameraKeyframe>, JsonDeserializer<CameraKeyframe> {
        ...
        public CameraKeyframe deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
            ...
            return new CameraKeyframe(position, rotation);
        }
